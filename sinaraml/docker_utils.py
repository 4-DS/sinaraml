...
def docker_volume_create(volume_name):
    try:
        ...
        logging.error(e, exc_info=True)
        raise Exception(
            f"Cannot create volume {volume_name}.\nCheck if you have permission to access docker volumes"
        )
...
def docker_volume_remove(volume_name):
    try:
        ...
        logging.error(e, exc_info=True)
        raise Exception(
            f"Cannot remove volume {volume_name}.\nCheck if you have permission to access docker volumes"
        )
...
def docker_container_running(container_name):
    try:
        ...
        logging.error(e, exc_info=True)
        raise Exception(
            f"Cannot check container {container_name} state.\nCheck if you have permission to access docker containers"
        )
    return False
...
def docker_container_create(image, command=None, **kwargs):
    try:
        ...
        logging.error(e, exc_info=True)
        raise Exception(
            f"Cannot create container from {image}.\nCheck if you have permission to access docker containers"
        )
...
def docker_container_start(container_name):
    try:
        ...
        logging.error(e, exc_info=True)
        raise Exception(
            f"Cannot start container {container_name}.\nCheck if you have permission to access docker containers"
        )
...
def docker_container_stop(container_name):
    try:
        ...
        logging.error(e, exc_info=True)
        raise Exception(
            f"Cannot stop container {container_name}.\nCheck if you have permission to access docker containers"
        )
...
def docker_container_pause(container_name):
    try:
        ...
        logging.error(e, exc_info=True)
        raise Exception(
            f"Cannot pause container {container_name}.\nCheck if you have permission to access docker containers"
        )
...
def docker_container_remove(container_name):
    try:
        ...
        logging.error(e, exc_info=True)
        raise Exception(
            f"Cannot remove container {container_name}.\nCheck if you have permission to access docker containers"
        )
...
def docker_container_exec(container_name, command):
    try:
        ...
        return container.exec_run(
            command, privileged=True, user="root", stream=False, demux=True
        )
    except Exception as e:
        logging.error(e, exc_info=True)
        raise Exception(
            f"Cannot exec command in container {container_name}.\nCheck if you have permission to access docker containers"
        )
...
def docker_copy_from_container(container_name, src_path, dest_path):
    try:
        ...
        logging.error(e, exc_info=True)
        raise Exception(
            f"Cannot copy files from container {container_name}.\nCheck if you have permission to access docker containers"
        )
...
def docker_pull_image(image):
    try:
        ...
        with tqdm.tqdm(unit=" b") as pbar:
            layers = {}
            for data in client.pull(image, stream=True, decode=True):
                progress = data.get("progressDetail")
                _id = data.get("id")

                if (_id is not None) and (progress is not None):
                    layers[_id] = progress
...
def docker_get_port_on_host(container_name, container_port):
    try:
        ...
        port_data = client.api.inspect_container(container.id)["NetworkSettings"][
            "Ports"
        ]
        ...
        logging.error(e, exc_info=True)
        raise Exception(
            f"Cannot get ports of container {container_name}.\nCheck if you have permission to access docker containers"
        )
...
def docker_get_container_labels(container_name):
    try:
        ...
        logging.error(e, exc_info=True)
        raise Exception(
            f"Cannot get labels of container {container_name}.\nCheck if you have permission to access docker containers"
        )
