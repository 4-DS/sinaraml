...
class SinaraModel:
    ...
    def add_command_handlers(root_parser):
        parser_model = root_parser.add_parser(
            SinaraModel.subject, help="sinara model subject"
        )
        model_subparsers = parser_model.add_subparsers(
    ...
    def add_containerize_handler(root_parser):
        model_containerize_parser = root_parser.add_parser(
            "containerize", help="containerize sinara bento service into a docker image"
        )
        model_containerize_parser.add_argument(
            "--instanceName",
            default=SinaraModel.server_container_name,
            help="Custom model container name to start (default: %(default)s)",
        )
        model_containerize_parser.add_argument(
            "--bentoservicePath", help="Path to bentoservice inside sinara server"
        )
        model_containerize_parser.add_argument(
            "--dockerRegistry", help="Docker registry url of a model image"
        )
        model_containerize_parser.set_defaults(func=SinaraModel.containerize)

    @staticmethod
    def add_extract_artifacts_handler(root_parser):
        model_extract_artifacts_parser = root_parser.add_parser(
            "extract_artifacts", help="extract artifacts from sinara model container"
        )
        model_extract_artifacts_parser.add_argument(
            "--modelImage", help="Docker model image to extract artifacts from"
        )
        model_extract_artifacts_parser.add_argument(
            "--extractTo", help="Folder on host, were to place extracted artifacts"
        )
    ...
    def save_extra_info(bentoservice_dir, image_tag):
        ...
        if len(file_paths) > 0:
            ...
            bentoservice_artifacts_dir = (
                Path(bentoservice_dir) / bento_service_class / "artifacts"
            )
            ...
            with artifacts_list_path.open(mode="w") as artifacts_list_file:
                for file_path in file_paths:
                    ...
                    md5_path = (
                        bentoservice_artifacts_dir / "checksum" / f"{file_p.stem}.md5"
                    )
                    with md5_path.open(mode="w") as md5_file:
                        md5_file.write(compute_md5(file_path))

        docker_image_info_path = (
            bentoservice_artifacts_dir / "checksum" / "docker_image_info.txt"
        )
    ...
    def containerize(args):
        ...
        if not args.bentoservicePath:
            while not args.bentoservicePath:
                args_dict["bentoservicePath"] = get_expanded_path(
                    input("Please, enter ENTITY_PATH for your bentoservice: ")
                )
        ...
        if not args.dockerRegistry:
            while not args.dockerRegistry:
                args_dict["dockerRegistry"] = input(
                    "Please, enter Docker registry address for your model image: "
                )
        ...
        docker_copy_from_container(
            args.instanceName,
            src_path=args.bentoservicePath,
            dest_path=bentoservice_cache_dir,
        )
        ...
        model_image_name_full = (
            f"{args.dockerRegistry}/{model_image_name}:{model_image_tag}"
        )
        ...
        docker_build_image(
            path=str(bentoservice_cache_dir),
            tag=model_image_name_full,
            pull=True,
            forcerm=True,
            rm=True,
            quiet=False,
        ...
        if not docker_container_exists(args.modelContainerName):
            if not args.modelImage:
                while not args.modelImage:
                    args_dict["modelImage"] = input(
                        "Please, enter your model image to run: "
                    )
            ...
            if not args.gpuEnabled:
                while model_type not in [1, 2]:
                    model_type = int(
                        input(
                            "Please, choose model type for this model 1) ML or 2) CV: "
                        )
                    )
                if model_type == 2:
                    args_dict["gpuEnabled"] = "y"
                    gpu_requests = [
                        types.DeviceRequest(count=-1, capabilities=[["gpu"]])
                    ]
    ...
    def stop(args):
        if not docker_container_exists(args.modelContainerName):
            raise Exception(
                f"Your model container with name {args.modelContainerName} doesn't exist"
            )
    ...
    def extract_artifacts(args):
        ...
        if not args.modelImage:
            while not args.modelImage:
                args_dict["modelImage"] = input(
                    "Please, input model image name with tag: "
                )

        if not args.extractTo:
            while not args.extractTo:
                args_dict["extractTo"] = get_expanded_path(
                    input("Please, input path to folder where to extract artifacts: ")
                )
        ...
        docker_copy_from_container(
            SinaraModel.model_container_name,
            src_path=artifacts_path,
            dest_path=args.extractTo,
        )
