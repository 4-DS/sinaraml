...
from .common_utils import get_expanded_path, get_public_ip
...
class SinaraServer:
    ...
    def add_command_handlers(root_parser, subject_parser):
        ...
        parser_server = subject_parser.add_parser(
            SinaraServer.subject, help="sinara server subject"
        )
        server_subparsers = parser_server.add_subparsers(
            title="action", dest="action", help="Action to do with subject"
        )
    ...
    def add_create_handler(server_cmd_parser):
        SinaraServer.create_parser = server_cmd_parser.add_parser(
            "create", help="create sinara server"
        )
        SinaraServer.create_parser.add_argument(
            "--instanceName",
            default=SinaraServer.container_name,
            type=str,
            help="sinara server container name (default: %(default)s)",
        )
        SinaraServer.create_parser.add_argument(
            "--runMode",
            default="q",
            choices=["q", "b"],
            help="Runmode, quick (q) - work, data, tmp will be mounted inside docker volumes, basic (b) - work, data, tmp will be mounted from host folders (default: %(default)s)",
        )
        SinaraServer.create_parser.add_argument(
            "--createFolders",
            default="y",
            choices=["y", "n"],
            help="y - create work, data, tmp folders in basic mode automatically, n - folders must be created manually (default: %(default)s)",
        )
        SinaraServer.create_parser.add_argument(
            "--gpuEnabled",
            choices=["y", "n"],
            help="y - Enables docker container to use Nvidia GPU, n - disable GPU",
        )
        SinaraServer.create_parser.add_argument(
            "--memRequest",
            default="4g",
            type=str,
            help="Amount of memory requested for server container (default: %(default)s)",
        )
        SinaraServer.create_parser.add_argument(
            "--memLimit",
            default="8g",
        ...
        SinaraServer.create_parser.add_argument(
            "--jovyanDataPath",
            type=str,
            help="Path to data fodler on host (only used in basic mode)",
        )
        SinaraServer.create_parser.add_argument(
            "--jovyanWorkPath",
            type=str,
            help="Path to work folder on host (only used in basic mode)",
        )
        SinaraServer.create_parser.add_argument(
            "--jovyanTmpPath",
            type=str,
            help="Path to tmp folder on host (only used in basic mode)",
        )
        SinaraServer.create_parser.add_argument(
            "--infraName",
            default=SinaraInfra.LocalFileSystem,
            choices=SinaraServer.get_available_infra_names(),
            type=str,
            help="Infrastructure name to use (default: %(default)s)",
        )
        SinaraServer.create_parser.add_argument(
            "--insecure",
            action="store_true",
            help="Run server without password protection",
        )
        SinaraServer.create_parser.add_argument(
            "--platform",
            default=SinaraPlatform.Desktop,
            choices=list(SinaraPlatform),
            type=SinaraPlatform,
            help="Server platform - host where the server is run",
        )
        SinaraServer.create_parser.add_argument(
            "--experimental", action="store_true", help="Use expermiental server images"
        )
        SinaraServer.create_parser.add_argument(
            "--image", type=str, help="Custom server image name"
        )
        SinaraServer.create_parser.set_defaults(func=SinaraServer.create)

    @staticmethod
    def add_start_handler(root_parser):
        server_start_parser = root_parser.add_parser(
            "start", help="start sinara server"
        )
    ...
    def add_remove_handler(root_parser):
        server_remove_parser = root_parser.add_parser(
            "remove", help="remove sinara server"
        )
        server_remove_parser.add_argument(
            "--instanceName",
            default=SinaraServer.container_name,
            help="sinara server container name (default: %(default)s)",
        )
        server_remove_parser.add_argument(
            ...
            default="n",
            choices=["y", "n"],
            help="y - remove existing data, work, tmp docker volumes, n - keep volumes  (default: %(default)s)",
        )
    ...
    def add_update_handler(root_parser):
        server_remove_parser = root_parser.add_parser(
            "update", help="update docker image of a sinara server"
        )
        server_remove_parser.add_argument(
            "--image",
            choices=["ml", "cv"],
            help="ml - update ml image, cv - update CV image",
        )
        server_remove_parser.add_argument(
            "--experimental",
            action="store_true",
            help="Update expermiental server images",
        )
    ...
    def create(args):
        ...
        if current_infra not in infras.keys():
            raise Exception(
                f'Infra "{current_infra}" is not supported, available: {", ".join(infras.keys())}'
            )
    ...
    def _create(args):
        ...
        if docker_container_exists(args.instanceName):
            print(
                f"Sinara server {args.instanceName} aleady exists, remove it and run create again"
            )
            return
        ...
        if not args.gpuEnabled:
            ...
            while sinara_image_num not in [1, 2]:
                try:
                    sinara_image_num = int(
                        input("Please, choose a Sinara for 1) ML or 2) CV projects: ")
                    )
                except ValueError:
                    pass
        ...
        if not args.image:
            sinara_image = SinaraServer.sinara_images[int(args.experimental)][
                int(sinara_image_num - 1)
            ]
        else:
        ...
        if args.insecure:
            server_cmd = (
                f"{server_cmd} --NotebookApp.token='' --NotebookApp.password=''"
            )
        ...
        docker_container_create(
            ...
            * int(
            ...
            labels={"sinaraml.platform": str(args.platform)},
    ...
    def _prepare_quick_mode(args):
        ...
        ensure_docker_volume(
            data_volume, already_exists_msg="Docker volume with jovyan data is found"
        )
        ensure_docker_volume(
            work_volume, already_exists_msg="Docker volume with jovyan work is found"
        )
        ensure_docker_volume(
    ...
    def _prepare_basic_mode(args):
        if args.createFolders == "y":
            if not args.jovyanRootPath:
                jovyan_root_path = input(
                    "Please, choose jovyan Root folder path (data, work and tmp will be created there): "
                )
            ...
            if not args.jovyanDataPath:
                jovyan_data_path = get_expanded_path(
                    input("Please, choose jovyan Data path: ")
                )
            if not args.jovyanWorkPath:
                jovyan_work_path = get_expanded_path(
                    input("Please, choose jovyan Work path: ")
                )
            if not args.jovyanTmpPath:
                jovyan_tmp_path = get_expanded_path(
                    input("Please, choose jovyan Tmp path: ")
                )
    ...
    def get_server_url(instance):
        ...
        for cmd in commands:
            ...
            log_lines = (
                ...
                else [*log_lines_stderr, *log_lines_stdout]
            )
    ...
    def get_server_platform(instance):
        ...
        if "sinaraml.platform" not in labels:
    ...
    def start(args):
        if not docker_container_exists(args.instanceName):
            print(
                f"Sinara server with name {args.instanceName} doesn't exist yet, run 'sinara server create' first"
            )
            return
        ...
        print(
            f"Sinara server {args.instanceName} started, platform: {platform}\n{server_hint}"
        )
        ...
        if not args.image:
            while sinara_image_num not in [1, 2]:
                try:
                    sinara_image_num = int(
                        input(
                            "Please, choose a Sinara image to update 1) ML or 2) CV projects: "
                        )
                    )
                except ValueError:
                    pass
