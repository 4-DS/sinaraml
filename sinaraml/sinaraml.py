...
def main():
    ...
    subject_subparser = parser.add_subparsers(
        title="subject", dest="subject", help="subject to use"
    )
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="display verbose logs"
    )
    parser.add_argument(
        "--version", action="version", version=f"SinaraML CLI {get_cli_version()}"
    )
